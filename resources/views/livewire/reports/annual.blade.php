    ...
    <div class="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <x-stat-card :label="__('Ahorro total')" icon="chart-pie">
                ...
                <x-slot name="footer">{{ number_format($summary['savingsRate'], 1) }}% {{ __('tasa de ahorro anual') }}</x-slot>
            </x-stat-card>
            <x-stat-card :label="__('Promedio mensual')" icon="calendar">
                <x-money :amount="$summary['avgMonthlyExpense']" />
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            @if ($summary['bestIncomeMonth'])
                <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <p class="mt-1 text-lg font-semibold capitalize text-gray-900 dark:text-white">{{ $summary['bestIncomeMonth']['label'] }}</p>
                    <p class="text-sm text-gray-500"><x-money :amount="$summary['bestIncomeMonth']['income']" /></p>
                </div>
            @endif
            @if ($summary['worstExpenseMonth'])
                <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Mes con mayores gastos') }}</p>
                    <p class="mt-1 text-lg font-semibold capitalize text-gray-900 dark:text-white">{{ $summary['worstExpenseMonth']['label'] }}</p>
                    <p class="text-sm text-gray-500"><x-money :amount="$summary['worstExpenseMonth']['expense']" /></p>
                </div>
            @endif
            @if ($summary['topCategory'])
                <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Categoría donde más gastaste') }}</p>
                    <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">{{ $summary['topCategory']['category']?->name }}</p>
                    <p class="text-sm text-gray-500">{{ number_format($summary['topCategory']['percentage'], 1) }}% {{ __('del total') }}</p>
                </div>
            @endif
        </div>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @foreach ($summary['months'] as $row)
                        <tr>
                            ...
                            <td class="px-4 py-2.5 text-right text-sm tabular-nums text-green-600 dark:text-green-400"><x-money :amount="$row['income']" /></td>
                            <td class="px-4 py-2.5 text-right text-sm tabular-nums text-red-600 dark:text-red-400"><x-money :amount="$row['expense']" /></td>
                            <td class="px-4 py-2.5 text-right text-sm font-medium tabular-nums text-gray-900 dark:text-white"><x-money :amount="$row['savings']" /></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
